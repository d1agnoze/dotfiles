...
git fetch origin
echo "🤖 Generating PR message using OpenCode CLI (Deepseek v4 Flash)..."
PR_MESSAGE=$(opencode run -m "opencode/deepseek-v4-flash-free" "Write a pull request body in plain text for merging branch '$CURRENT_BRANCH' into 'dev'. Base it on the git commits and diff between '$CURRENT_BRANCH' and 'origin/dev'. Return only the PR body with a short summary and bullet points for the most important changes.")
